...
def load_filter_options():
    ...
    with engine.connect() as conn:
        provincias = (
            pd.read_sql(
                text(
                    "SELECT DISTINCT provincia FROM alertas "
                    "WHERE prioridad > 100 AND feedback IS NULL "
                    "ORDER BY provincia"
                ),
                conn,
            )["provincia"]
        ...
        bloques = (
            pd.read_sql(
                text(
                    "SELECT DISTINCT bloque::TEXT FROM alertas "
                    "WHERE prioridad > 100 AND feedback IS NULL "
                    "ORDER BY bloque::TEXT"
                ),
                conn,
            )["bloque"]
            .tolist()
        ...
        familias_df = pd.read_sql(
            text(
                "SELECT DISTINCT bloque::TEXT AS bloque, familia FROM alertas "
                "WHERE prioridad > 100 AND feedback IS NULL "
                "ORDER BY bloque, familia"
            ),
            conn,
        )
...
def load_dashboard_metrics():
    ...
    with engine.connect() as conn:
        ...
        m["distribution"] = pd.read_sql(
            text(
                "SELECT tipo_alerta, COUNT(*) AS n FROM alertas "
                "WHERE prioridad > 100 AND feedback IS NULL "
                "GROUP BY tipo_alerta ORDER BY n DESC"
            ),
            conn,
        )

        m["history"] = pd.read_sql(
            text(
        ...
        m["feedback_breakdown"] = pd.read_sql(
            text(
        ...
        m["by_block"] = pd.read_sql(
            text(
                "SELECT bloque::TEXT, tipo_alerta, COUNT(*) AS n FROM alertas "
                "WHERE prioridad > 100 AND feedback IS NULL "
                "GROUP BY bloque, tipo_alerta ORDER BY bloque, n DESC"
            ),
            conn,
        )
...
def load_alerts(provincia, tipo_list, bloque, familia, min_pri, max_pri, search_text):
    engine = get_engine()
    ...
    params["min_p"] = min_pri + 100
    params["max_p"] = max_pri + 100

    if search_text:
        conditions.append("LOWER(id_cliente) LIKE :srch")
        params["srch"] = f"%{search_text.lower()}%"
...
def submit_feedback(alert_id, value):
    ...
    with engine.connect() as conn:
        with conn.begin():
            conn.execute(
                text(
                    "UPDATE alertas SET feedback = :val, fecha_feedback = CURRENT_DATE "
                    "WHERE id_alerta = :aid"
                ),
...
def alert_badge_html(tipo):
    ...
    return (
        f'<span style="background:{c}20;color:{c};padding:3px 10px;'
...
def profile_badge_html(perfil):
    ...
    return (
        ...
        f'border-radius:12px;font-size:0.73rem">{perfil.title()}</span>'
    )


def flag_dot(active, label):
    c = "#4caf50" if active else "#444"
    return f'<span style="color:{c};white-space:nowrap">{chr(9679) if active else chr(9675)} {label}</span>'
...
def render_dashboard():
    ...
    with col_a:
        ...
        if not dist.empty:
            fig = go.Figure(
                data=[
                    go.Pie(
                        labels=dist["tipo_alerta"]
                        .str.replace("_", " ")
                        .str.title(),
                        values=dist["n"],
                        hole=0.45,
                        marker=dict(
                            colors=[ALERT_COLORS.get(t, "#666") for t in dist["tipo_alerta"]],
            ...
            fig.update_layout(
                title=dict(text="Distribución por Tipo de Alerta", font=dict(color="#e0e6ed", size=15)),
    ...
    with col_b:
        ...
        if not prov.empty:
            ...
            fig2.update_layout(
                title=dict(text="Top 10 Provincias", font=dict(color="#e0e6ed", size=15)),
                paper_bgcolor="rgba(0,0,0,0)",
                plot_bgcolor="rgba(0,0,0,0)",
                height=400,
                yaxis=dict(autorange="reversed", color="#8fa3b8"),
    ...
    if not history.empty:
        ...
        all_dates = pd.date_range(
            start=date.today() - timedelta(days=13), end=date.today(), freq="D"
        )
        ...
        fig3.update_layout(
            title=dict(text="Alertas Generadas (14 días)", font=dict(color="#e0e6ed", size=15)),
            paper_bgcolor="rgba(0,0,0,0)",
            plot_bgcolor="rgba(0,0,0,0)",
            height=320,
            xaxis=dict(color="#8fa3b8", gridcolor="#1e3a5f", dtick=86400000 * 2 * 1000 if False else None),
    ...
    with col_c:
        st.metric("\u2705 Feedback Positivo (30d)", pos)
    with col_d:
        st.metric("\u274C Feedback Negativo (30d)", neg)
...
def render_alerts():
    ...
    with st.sidebar:
        ...
        provincia_filter = st.selectbox(
            "Provincia", options=["Todas"] + provincias, key="filtro_prov"
        )
        ...
        bloque_filter = st.selectbox(
            "Bloque", options=["Todos"] + bloques, key="filtro_bloque"
        )

        fam_options = ["Todas"]
        if bloque_filter != "Todos":
            fam_options += (
                familias_df[familias_df["bloque"] == bloque_filter]["familia"]
                .tolist()
            )
        else:
            fam_options += familias_df["familia"].tolist()
        familia_filter = st.selectbox(
            "Familia", options=fam_options, key="filtro_fam"
        )

        st.markdown("---")
        min_pri, max_pri = st.slider(
            "Rango de Prioridad (UI 0\u2013100)",
            min_value=0,
            max_value=100,
            value=(0, 100),
            step=5,
        ...
        search_filter = st.text_input(
            "\U0001F50E Buscar Cliente", placeholder="ID de cl\u00ednica...", key="filtro_search"
        )
    ...
    tipos = tipo_filter if tipo_filter else None

    with st.spinner("Cargando alertas..."):
        df = load_alerts(prov, tipos, bloq, fam, min_pri, max_pri, search_filter)
    ...
    with st.sidebar:
        st.download_button(
            label="\U0001F4C4 Descargar CSV",
            data=csv_data,
            file_name=f"alertas_inibsa_{date.today().isoformat()}.csv",
            mime="text/csv",
        )
    ...
    st.markdown(
        f"**{len(client_order)}** cl\u00ednicas con **{len(df)}** alertas encontradas"
    )
    st.markdown("---")
    ...
    for client_id in client_order:
        ...
        child_str = f"  +{n_child} m\u00e1s" if n_child else ""

        expander_label = (
            f"{top_icon} Cliente {client_id}  |  "
        ...
        with st.expander(expander_label):
            _render_alert_card(top, is_main=True)
            ...
            if children is not None and len(children) > 0:
                st.markdown("---")
                st.markdown(f"### \U0001F53D Otras {len(children)} alertas para este cliente")
                ...
                for _, child in children.iterrows():
                    ...
                    child_exp = (
                        ...
                        f"P:{int(child['prioridad_ui'])}  |  "
                        f"{child['familia']}  |  "
                        f"{str(child['motivo'])[:100]}..."
                    )
                    with st.expander(child_exp):
                        _render_alert_card(child, is_main=False)
                        st.markdown("---")
                        _render_feedback_buttons(child["id_alerta"], prefix="c")


def _render_alert_card(alert, is_main=True):
    ...
    st.markdown(
        ...
        <strong style="color:{color};font-size:1.05rem">{icon} {tipo_label}</strong>
        <p style="color:#b0b8c4;margin:4px 0 0 0;font-size:0.9rem">{alert['motivo']}</p>
        </div>""",
        unsafe_allow_html=True,
    )
    ...
    flag_html = "&nbsp;&nbsp;".join(
        [
            flag_dot(alert.get("alerta_frecuencia", False), "Frecuencia"),
            flag_dot(alert.get("alerta_volumen", False), "Volumen"),
            flag_dot(alert.get("alerta_ausencia", False), "Ausencia"),
            flag_dot(alert.get("alerta_anomalia", False), "Anomal\u00eda"),
        ]
    )
    st.markdown(
        f'<div style="margin-bottom:10px;font-size:0.82rem">{flag_html}</div>',
        unsafe_allow_html=True,
    )
    ...
    with col_left:
        ...
        mc1.metric("Prioridad", f"{_fmt(alert['prioridad_ui'])}/100")
        mc2.metric("Score Conv.", f"{(_fmt(alert.get('score_conversion'), 2) or 0) * 100:.0f}%")
        mc3.metric("Potencial", f"\u20AC{_fmt(alert.get('potencial_h')) or 0:,}")
        mc4.metric("Impacto Est.", f"\u20AC{_fmt(alert.get('impacto_estimado')) or 0:,}")

        mc5, mc6, mc7, mc8 = st.columns(4)
        mc5.metric("D\u00edas sin compra", _fmt(alert.get("dias_desde_ultima_compra")))
        mc6.metric("Urgencia (d\u00edas)", _fmt(alert.get("urgencia_dias")))
        mc7.metric("Promiscuidad", f"{(_fmt(alert.get('ratio_promiscuidad'), 2) or 0) * 100:.0f}%")
        mc8.metric("Freq. Media (d)", _fmt(alert.get("freq_media_dias"), 1))

        perfil = alert.get("perfil_cliente", "")
        st.markdown(
            f"Perfil: {profile_badge_html(perfil)} &nbsp;|&nbsp; "
            f"Compras hist\u00f3ricas: {_fmt(alert.get('n_compras_hist'))} &nbsp;|&nbsp; "
            f"ID alerta: <small>{alert['id_alerta']}</small>",
            unsafe_allow_html=True,
        )
    ...
    with col_right:
        ...
        if not purchase_df.empty:
            ...
            fig.update_layout(
                title=dict(
                    text=f"Historial: {alert['familia']}",
                    font=dict(size=12, color="#8fa3b8"),
                ),
                paper_bgcolor="rgba(0,0,0,0)",
                plot_bgcolor="rgba(0,0,0,0)",
                height=240,
                ...
                margin=dict(t=35, b=5, l=45, r=10),
            )
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.caption("Sin historial de compras disponible.")


def _render_feedback_buttons(alert_id, prefix=""):
    st.markdown("#### Acci\u00f3n Comercial")
    fb1, fb2, fb3 = st.columns([1, 1, 2.5])
    with fb1:
        if st.button(
            "\u2705 Venta Conseguida", key=f"{prefix}pos_{alert_id}", type="primary"
        ):
            submit_feedback(alert_id, 1)
            st.session_state.fb_done.add(alert_id)
            st.success("Feedback registrado. \u00a1Venta conseguida!")
            st.rerun()
    with fb2:
        if st.button("\u274C Sin Inter\u00e9s", key=f"{prefix}neg_{alert_id}"):
            submit_feedback(alert_id, 0)
            st.session_state.fb_done.add(alert_id)
            st.info("Feedback registrado. Sin inter\u00e9s.")
            st.rerun()
...
def main():
    ...
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:8px">
        <h1 style="color:#2196F3;margin:0">🦷 Smart Demand Signals</h1>
    </div>
    <p style="color:#8fa3b8;font-size:1.05rem;margin-bottom:16px">
